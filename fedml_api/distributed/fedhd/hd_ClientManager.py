...
class HDClientManager(ClientManager):
    ...
    def handle_message_init_from_server(self, msg_params):
        ...
        global_class_hypervectors = msg_params.get(MyMessage.MSG_ARG_KEY_MODEL_PARAMS)
        client_index = msg_params.get(MyMessage.MSG_ARG_KEY_CLIENT_INDEX)
        self.download_epoch = msg_params.get(MyMessage.MSG_ARG_KEY_DOWNLOAD_EPOCH)

        self.trainer.update_model(global_class_hypervectors)
        self.round_idx = 0
        self.__train()
    ...
    def handle_message_receive_model_from_server(self, msg_params):
        ...
        self.trainer.update_model(global_class_hypervector)
    ...
    def send_model_to_server(self, receive_id, class_hypervectors, local_sample_num,
                             local_loss, local_comp_delay):

        message = Message(MyMessage.MSG_TYPE_C2S_SEND_MODEL_TO_SERVER, self.get_sender_id(), receive_id)
        message.add_params(MyMessage.MSG_ARG_KEY_MODEL_PARAMS, class_hypervectors)
    ...
    def __train(self):
        ...
        start = time.time()
        class_hypervectors, local_sample_num, local_loss = self.trainer.train()
        local_comp_delay = time.time() - start
        self.send_model_to_server(0, class_hypervectors, local_sample_num,
                                  local_loss, local_comp_delay)
