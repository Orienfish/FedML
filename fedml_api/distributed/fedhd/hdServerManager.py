...
def terminate():
    while True:
        ...
        if not running:
            try:
                for line in os.popen('ps aux | grep app_HD.py | grep -v grep'):
                    fields = line.split()
                    pid = fields[1]
...
class HDServerManager(ServerManager):
    ...
    def handle_message_receive_model_from_client_sync(self, msg_params):
        ...
        class_hypervectors = msg_params.get(MyMessage.MSG_ARG_KEY_MODEL_PARAMS)
        ...
        self.aggregator.add_local_trained_result(sender_id - 1, class_hypervectors, local_sample_number)
    ...
    def handle_message_receive_model_from_client_async(self, msg_params):
        ...
        if self.warmup_done:
            ...
            self.aggregator.aggregate_async(class_hypervectors, local_sample_number, staleness)
